...
interface UserFeedSubscriptionsStoreState {
  ...
  readonly recentSubscriptionsCacheState: AsyncState<
    Record<UserFeedSubscriptionId, UserFeedSubscription>
  >;
...
export const useUserFeedSubscriptionsStore = create<UserFeedSubscriptionsStoreState>((set) => ({
  // Initial state.
  ...
  registerService: (args) => {
    ...
    const unsubscribe = userFeedSubscriptionsService.watchSubscriptions({
      ...
      onData: (subscriptions) => {
        ...
        set({recentSubscriptionsCacheState: makeSuccessAsyncState(subscriptions)});
      },
      onError: (error) => {
